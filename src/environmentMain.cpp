    ...
    rng(base_seed), cell_grid(100, 100)
{
     ...
     * -----------------------------------
     * loads the three parameter files (cell parameters, environment parameters, recruitment parameters)
    ...
    cellParams[12][2] = 0.9897; // Fixed kdeath
    cellParams[13][2] = 0.1; // CD8 ccl exh / kd_prol (ccl)
    ...
    // {CD8, macrophage, CD4, NK, MDSC} -> same order as RecRates, values for CELL_STATE
    immuneCellRecTypes = {3, 1, 2, 4, 5};
...
void Environment::simulate(double tstep, int tx, int met, double bind_rate_pd1_drug, bool onLocal)
{
    ...
    while (tstep * steps / 24 < simulationDuration)
    {
        ...
        // loops through twice, calculates the tumor center, calculates the furthest distance from the center to a cancer cell.
        steps += 1;
        ...
        {   // change the y value depending on how frequently you want the stuff to be saved.
