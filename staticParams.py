...
# USING THIS FILE WILL DISABLE ALL BIOLOGY EXCEPT FOR FORCES

# Immune synapse formation has been disabled
# in the file <<src/environmentRunCells.cpp>>

# Macrophage differentiation has also been disabled
# in the file <<src/CellTypes/Macrophage.cpp>>

# Cell lifespans have been set to arbitrarily large amounts (5000 days)
# Additionally cell aging has been disabled in <<src/RS_Cell.cpp>>
# You can verify that no deaths occur by examining time series

# Cancer migration disabled in <<src/CellTypes/Cancer.cpp>>
# Cancer proliferation disabled in <<src/CellTypes/Cancer.cpp>>

# CD8 proliferation disabled in <<src/CellTypes/CD8.cpp>>
...
cd8RecRate = 0 # 0.00192 # these are the percentages of T cells from non-tx tumors provided by ERT
cd4RecRate = 0 # 0.00192 # these are the percentages of T cells from non-tx tumors provided by ERT
macRecRate = 0 # 0.00125 # these are the percentages of mature myeloid cells from non-tx tumors provided by ERT
nkRecRate = 0 # 0.00044 # these are the percentages of NK cells from non-tx tumors provided by ERT
mdscRecRate = 0 # 0.00469 # these are the percentages of MDSCs from non-tx tumors provided by ERT
...
necrosisLimit = 940.3
nkKillProb = 0 # This has been arbitrarily chosen.
...
cellParams[6, 0] = 1/(24.0*5000.0) # death probability (hours)
...
cellParams[5, 1] = 1/(24.0*5000.0) # lifespan (hours) # Gong 2017 (for CD8 cells)
cellParams[6, 1] = 0  # migration speed base um/hr
cellParams[7, 1] = 0  # differentiation to Treg
...
cellParams[5, 2] = 1/(24.0*5000.0) # 1/lifespan (hours) https://pmc.ncbi.nlm.nih.gov/articles/PMC4489929/
cellParams[6, 2] = 0  # migration speed um/hr |
cellParams[7, 2] = 0 # killProb
...
cellParams[11, 2] = 0 # only for testing #0.053  # proliferation prob
...
cellParams[5, 3] = 1/(24.0*5000) # (set based on cd8) lifespan (days) -> https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4040600/ | Petty and Yang, Tumor-associated macrophages: implications in cancer immunotherapy, 2017
cellParams[6, 3] = 0  # migration speed um/hr |
cellParams[7, 3] = macM1  # kM1 (change these???)
cellParams[8, 3] = macM2  # kM2 (change these???)
...
cellParams[4, 4] = 7.0  # diameter (um) NK cells are actually 6-7 microns: https://pmc.ncbi.nlm.nih.gov/articles/PMC4566154/
cellParams[5, 4] = 1/(24.0*5000.0) # lifespan (days) -> 10.1111/j.1365-2567.2007.02573.x
cellParams[6, 4] = 0.0  # migration speed um/hr
cellParams[7, 4] = 0  # killProb
...
cellParams[5, 5] = 1/(24.0*5000.0)  # lifespan (days) https://pmc.ncbi.nlm.nih.gov/articles/PMC5765878/
cellParams[6, 5] = 0.0 # migration speed um/hr (assumed to be the same as other cells)
...
envParams[1] = 25.0 # simulation duration (days)
