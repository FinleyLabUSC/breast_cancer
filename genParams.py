...
tcellMigBias = 0.7 #0.076
cd4Diff = 0.9
macMigBias = 0.3
mdscMigBias = 0.4
nkMigBias = 0.5#0.125
macM1 = 0.5 # This needs to be fine tuned, but the data from ERT shows more than double M1 than M2
macM2 = 0.5 # This needs to be fine tuned, but the data from ERT shows more than double M1 than M2
cd8RecRate = 0.2#0.1# 0.00192 # these are the percentages of T cells from non-tx tumors provided by ERT
cd4RecRate = 0.2#0.1# 0.00192 # these are the percentages of T cells from non-tx tumors provided by ERT
macRecRate = 0.2#0.1 # 0.00125 # these are the percentages of mature myeloid cells from non-tx tumors provided by ERT
nkRecRate = 0.1#0.1 # 0.00044 # these are the percentages of NK cells from non-tx tumors provided by ERT
mdscRecRate = 0.3#0.1 # 0.00469 # these are the percentages of MDSCs from non-tx tumors provided by ERT
recDelay = 0.00
necroticGrowth = 0 #params[12]
necrosisLimit = 940.3
nkKillProb = 0.1#1#0.7 # This has been arbitrarily chosen.
...
cellParams[6, 1] = 200#200.0  # migration speed base um/hr
...
cellParams[5, 2] = 1/(24.0*5.0) # 1/lifespan (days) https://pmc.ncbi.nlm.nih.gov/articles/PMC4489929/
...
cellParams[7, 2] = 0.1#0.8 # killProb
...
cellParams[11, 2] = 0.5 # proliferation prob
...
cellParams[5, 4] = 1/(24.0*5.0) # lifespan (days) -> 10.1111/j.1365-2567.2007.02573.x
cellParams[6, 4] = 100#200.0  # migration speed um/hr | https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5702045/ -> scaled based on model scale
...
envParams[0] = 3.0  # initTumorSize x | circle radius
